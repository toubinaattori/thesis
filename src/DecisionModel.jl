...
function expected_value(model::Model,
    ...
    x_x::Dict{Tuple{Node,Node},VariableRef},
    z::DecisionVariables)
    @expression(model, sum(x * diagram.U(s, diagram.translation) * prod(z_d[s[[Int64(diagram.I_j[d]...),Int64(d)]]...] for (d,z_d) in zip(z.D, z.z)) for (s, x) in x_s) - sum(diagram.Cs[k] * x for (k,x) in x_x )+ sum(0.000001 * x for (k,x) in x_x ))
end
